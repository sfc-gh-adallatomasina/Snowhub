...
CREATE OR REPLACE TASK task_load_replication_group_usage_history_hst
--WAREHOUSE = &{l_target_wh}
USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
AFTER &{l_target_db}.&{l_target_schema}.task_load_replication_group_usage_history_stg
AS
EXECUTE IMMEDIATE $$
BEGIN 
 ALTER SESSION SET TIMEZONE = 'Europe/London'; 
insert into
    &{l_target_db}.&{l_target_schema}.replication_group_usage_history
with l_stg as
(
    select
        -- generate hash key to streamline processing
         sha1_binary( concat( s.account_name
                             ,s.organization_name
                             ,s.region_name
                             ,'|', to_char( NVL(s.replication_group_name, 'UNKNOWN') )
                             ,'|', to_char( ifnull( s.replication_group_id, -1 ) )
                             ,'|', to_char(convert_timezone('Europe/London',s.end_time), 'yyyy-mmm-dd hh24:mi:ss.FF3 TZHTZM'  )
                            )
                    )               as dw_event_shk
        ,s.*
    from
        &{l_target_db}.&{l_target_schema}.replication_group_usage_history_STG s
        where
...
where
    ...
    (
        select dw_event_shk from &{l_target_db}.&{l_target_schema}.replication_group_usage_history
    )
order by
