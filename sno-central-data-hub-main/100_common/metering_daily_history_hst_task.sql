...
CREATE OR REPLACE TASK task_load_metering_daily_history_hst
--WAREHOUSE = &{l_target_wh}
USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
AFTER &{l_target_db}.&{l_target_schema}.task_load_metering_daily_history_stg
AS
EXECUTE IMMEDIATE $$
BEGIN 
 ALTER SESSION SET TIMEZONE = 'Europe/London'; 
insert into
    &{l_target_db}.&{l_target_schema}.metering_daily_history
with l_stg as
(
    select
        ...
        ,s.*
    from
        &{l_target_db}.&{l_target_schema}.metering_daily_history_stg s
          where
...
where
    ...
    (
        select dw_event_shk from &{l_target_db}.&{l_target_schema}.metering_daily_history 
    )
order by
