...
CREATE OR REPLACE TASK task_load_stage_storage_usage_history_stg
...
TRUNCATE TABLE &{l_target_db}.&{l_target_schema}.stage_storage_usage_history_stg;
...
l_row_count := (SELECT COUNT(*)
                   ...
                   where  convert_timezone('Europe/London',usage_date)::timestamp_ntz >= to_timestamp(
                      select  ifnull( dateadd( hour, -4, max( usage_date ) ), '2020-01-01' ) as last_control_dt
                        from   &{l_target_db}.&{l_target_schema}.stage_storage_usage_history));

INSERT into
    &{l_target_db}.&{l_target_schema}.stage_storage_usage_history_stg
    (ORGANIZATION_NAME,ACCOUNT_NAME,REGION_NAME,USAGE_DATE,
    AVERAGE_STAGE_BYTES ,DW_FILE_NAME,DW_FILE_ROW_NO,DW_LOAD_TS)
    ...
    where  convert_timezone('Europe/London', s.usage_date)::timestamp_ntz >= to_timestamp(
              select  ifnull( dateadd( hour, -4, max( usage_date ) ), '2020-01-01' ) as last_control_dt
                from  &{l_target_db}.&{l_target_schema}.stage_storage_usage_history)  
