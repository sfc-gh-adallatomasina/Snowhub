...
CREATE  OR REPLACE TASK task_load_automatic_clustering_history_stg
...
TRUNCATE TABLE &{l_target_db}.&{l_target_schema}.automatic_clustering_history_stg;
...
l_row_count := (SELECT COUNT(*)
                   ...
                   to_timestamp(
                     select ifnull( dateadd( hour, -4, max( start_time ) ), '2020-01-01' ) as last_control_dt
                       from &{l_target_db}.&{l_target_schema}.automatic_clustering_history) ); 


insert into
    &{l_target_db}.&{l_target_schema}.automatic_clustering_history_stg
(ORGANIZATION_NAME,ACCOUNT_NAME,REGION_NAME,START_TIME,END_TIME,CREDITS_USED,
NUM_BYTES_RECLUSTERED,NUM_ROWS_RECLUSTERED,TABLE_ID,TABLE_NAME,SCHEMA_ID,SCHEMA_NAME,DATABASE_ID,DATABASE_NAME,
