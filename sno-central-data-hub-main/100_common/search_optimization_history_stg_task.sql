...
CREATE OR REPLACE TASK task_load_search_optimization_history_stg
...
TRUNCATE TABLE &{l_target_db}.&{l_target_schema}.search_optimization_history_stg ;
...
l_row_count := (SELECT COUNT(*)
                  ...
                  where  convert_timezone('Europe/London',start_time)::timestamp_ntz >= to_timestamp(
                       select   ifnull( dateadd( hour, -4, max( start_time ) ), '2020-01-01' ) as last_control_dt
                         from  &{l_target_db}.&{l_target_schema}.search_optimization_history )); 

INSERT into
    &{l_target_db}.&{l_target_schema}.search_optimization_history_stg       
