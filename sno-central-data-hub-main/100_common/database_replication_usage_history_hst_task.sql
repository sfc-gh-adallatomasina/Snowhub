--------------------------------------------------------------------
--  Purpose: create task to load history table
--
--  Revision History:
...
CREATE OR REPLACE TASK task_load_database_replication_usage_history_hst
--WAREHOUSE = &{l_target_wh}
USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
AFTER &{l_target_db}.&{l_target_schema}.task_load_database_replication_usage_history_stg
AS
EXECUTE IMMEDIATE $$
BEGIN 
 ALTER SESSION SET TIMEZONE = 'Europe/London'; 
insert into
    &{l_target_db}.&{l_target_schema}.database_replication_usage_history
with l_stg as
(
    select
        ...
        ,s.*
    from
        &{l_target_db}.&{l_target_schema}.database_replication_usage_history_stg s
        where s.start_time >= to_timestamp( select ifnull( dateadd( hour, -4, max( start_time ) ), '2020-01-01' ) as last_control_dt
...
where
    ...
    (
        select dw_event_shk from &{l_target_db}.&{l_target_schema}.database_replication_usage_history
    )
order by
