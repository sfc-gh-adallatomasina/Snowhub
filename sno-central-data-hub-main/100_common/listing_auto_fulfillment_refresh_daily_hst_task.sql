...
CREATE OR REPLACE TASK task_load_listing_auto_fulfillment_refresh_daily_hst
--WAREHOUSE = &{l_target_wh}
USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
AFTER &{l_target_db}.&{l_target_schema}.task_load_listing_auto_fulfillment_refresh_daily_stg
AS
EXECUTE IMMEDIATE $$
BEGIN 
...
INSERT INTO &{l_target_db}.&{l_target_schema}.listing_auto_fulfillment_refresh_daily_history
with l_stg as
(
    select
        -- generate hash key and hash diff to streamline processing
         sha1_binary( concat( s.account_name
                             ...
                             ,'|', to_char(NVL(s.FULFILLMENT_GROUP_NAME, 'UNKNOWN') )
                             ,'|', to_char(NVL(s.DATABASES, array_construct('UNKNOWN')) )
                             ,'|', to_char(NVL(s.LISTINGS, array_construct('UNKNOWN')) )
                            )
                    )   as dw_event_shk
        , s.*
    from  &{l_target_db}.&{l_target_schema}.listing_auto_fulfillment_refresh_daily_stg s      
)
...
from  l_stg s
where s.dw_event_shk not in ( select dw_event_shk from &{l_target_db}.&{l_target_schema}.listing_auto_fulfillment_refresh_daily_history )
